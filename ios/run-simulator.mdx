...
Your iOS instance is `ready`. From here you can read the screen, drive the UI with taps and typing, install and launch apps, capture screenshots and video, stream logs, and tunnel local services into the simulator. The same primitives compose into server-side batches that an agent uses to run a multi-step flow in a single round-trip.

For provisioning, see the [Quickstart](/quickstart); for builds, see [Build with remote Xcode](/ios/build-with-xcode). To pick a different model, region, or pre-installed app, jump to [Configure the instance](#configure-the-instance) near the bottom of the page.
