...
Once your instance is created and `ready`, this page covers everything you can do to it. For provisioning, see the [Quickstart](/quickstart). For builds, see [Build with remote Xcode](/ios/build-with-xcode).
...
Once the instance is `ready`, open a WebSocket client to control it. The TypeScript SDK covers every operation. The Go SDK covers most of them (screenshot, element-tree, tap, tap-element, typeText, pressKey, launchApp, listApps, openUrl, installApp, lsof, setOrientation, simctl streaming, plus element helpers). Python and Java only ship the control plane (creating, listing, deleting instances). For device control in those languages, use the CLI or raw REST. See the [SDK capability matrix](/reference#sdk-capability-matrix) for the full breakdown.
...
`createInstanceClient` (TypeScript) resolves once the WebSocket has connected and the initial device-info fetch has finished. After that, `client.deviceInfo` is populated synchronously with `udid`, `screenWidth`, `screenHeight`, and `model`. The Go `iosws.NewClient` returns the client immediately and connects lazily. The first call blocks until the WebSocket is up.
...
| `mcpUrl` | Per-instance MCP HTTP endpoint. Auth with your org API key, not `status.token`. See [MCP](/agents/mcp). |
