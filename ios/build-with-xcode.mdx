...
|---|---|---|---|---|---|
| Provision an Xcode instance | ✓ | ✓ | via REST | via REST | ✓ |
| Sync source and run `xcodebuild` | ✓ | not in SDK | not in SDK | not in SDK | ✓ |

Python and Go users provision the instance with their SDK and drive the build with `lim xcode build`. Java users provision with `lim ios create --xcode` since the Java SDK doesn't yet support the paired-with-Xcode shape. See the [SDK capability matrix](/reference#sdk-capability-matrix).
...
```java Java
// Plain iOS instance only; the Java SDK doesn't yet type the xcode sandbox.
// See the note below for the paired-with-Xcode flow.
...
The Java SDK doesn't yet support the paired-with-Xcode flow. Provision the paired instance with `lim ios create --xcode` and read the URL from the CLI or REST.
...
The **Go and Java SDKs don't yet have a standalone Xcode resource.** From those languages, either provision an iOS instance with the Xcode sandbox attached (the paired pattern above), POST to `/v1/xcode_instances` directly, or call `lim xcode create` from your process.
...
The build result includes a signed download URL when you've configured an upload. Store it in your PR comment or pipeline output. Anyone with the URL can download the IPA without a Limrun API key. The signature is short-lived, so hand it off promptly.
