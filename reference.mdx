...
The org API key must never ship to a browser or end-user client. For embedded simulators, your backend holds the API key and passes only `endpointWebSocketUrl` + `status.token` to `<RemoteControl />`. See [Embed the simulator](/platform/embed-simulator#authentication-model).
...
| iOS app-log / syslog streaming | ✓ | not supported | not supported |
